...
// TOTAL ACTIVE USERS (exclude archived)
$total_users = $conn->query("SELECT COUNT(*) FROM users WHERE account_status = 1 AND is_archived = 0")->fetch_row()[0] ?? 0;

// TOTAL EQUIPMENT (exclude archived)
$total_equipment = $conn->query("SELECT COUNT(*) FROM equipment WHERE is_archived = 0")->fetch_row()[0] ?? 0;

// EQUIPMENT WITH ERROR (exclude archived)
$equipment_with_error = $conn->query("SELECT COUNT(*) FROM equipment WHERE status = 'With Error' AND is_archived = 0")->fetch_row()[0] ?? 0;

// PENDING REQUESTS (safe check - table may not exist yet)
$pending_requests = 0;
